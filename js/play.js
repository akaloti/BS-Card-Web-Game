...
/*
    ...
    @returns array of submitted cards that have been taken from the
    player represented by bs.currentPlayerIndex
    @throws nothing
*/
...
/*
    ...
    computer opponent through the following algorithm: if the opponent
    can be honest, it will be; otherwise, it'll randomly choose cards
    @throws nothing
*/
function getCardSubmissionOfComputerOpponent() {
...
/*
    ...
    @returns array of cards, each with the rank indicated by rank,
    that have been taken away from the player indicated by indexOfPlayer
    @throws nothing
*/
function removeCardsOfRank(rank, indexOfPlayer) {
    var removedCards = [];
    for (var i = 0; i < bs.players[indexOfPlayer].cards.length; ) {
        if (bs.players[indexOfPlayer].cards[i].rank === rank)
            removedCards.push(
                bs.players[indexOfPlayer].cards.splice(i, 1).pop());
        else
            ++i;
    }
    return removedCards;
